...
type Selectors map[string]StringMap
type PointsMapFile struct {
	Selectors Selectors `json:"selectors"`
	PointsMap PointsMap `json:"points"`
}

func (s *Selectors) Get(selector string) StringMap {
...
func ImportPoints(filename string, parentId string) (PointsMap, error) {
	var pm PointsMapFile
	var err error

	for range Only.Once {
		err = output.FileRead(filename, &pm)
		if err != nil {
			break
		}
