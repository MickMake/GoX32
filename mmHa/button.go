...
func (m *Mqtt) PublishButtonConfig(config EntityConfig) error {
	...
	for range Only.Once {
		...
		payload := Button {
			...
			Qos:                    0,
			StateTopic:             JoinStringsForTopic(m.buttonPrefix, config.StateTopic, StateTopicSuffix),
			UniqueId:               config.StateTopic,

			CommandTopic:           JoinStringsForTopic(m.buttonPrefix, config.StateTopic, CmdTopicSuffix),
			Retain:                 true,
			ValueTemplate:          config.ValueTemplate,

			PayloadPress:           fmt.Sprintf(`{"%s":"%s"}`, config.Name, api.On),
			// PayloadPress:           config.ValueTemplate,
		}

		ct := JoinStringsForTopic(m.buttonPrefix, uid, ConfigTopicSuffix)
...
func (m *Mqtt) PublishButtonValue(config EntityConfig) error {
	...
	for range Only.Once {
		...
		st = JoinStringsForTopic(m.buttonPrefix, st, StateTopicSuffix)
...
func (m *Mqtt) PublishButtonValues(configs []EntityConfig) error {
	for range Only.Once {
		if len(configs) == 0 {
			break
		}
		...
		for i, config := range configs {
			...
			if topic == "" {
				topic = JoinStringsForTopic(m.buttonPrefix, config.StateTopic, StateTopicSuffix)
			}
...
type Button struct {
	...
	// Unique fields.
	PayloadPress             string       `json:"payload_press,omitempty" required:"false"`
