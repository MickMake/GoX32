...
func (m *Mqtt) PublishSelectConfig(config EntityConfig) error {
	...
	for range Only.Once {
		...
		payload := Select {
			...
			Qos:                    0,
			StateTopic:             JoinStringsForTopic(m.selectPrefix, config.StateTopic, StateTopicSuffix),
			UniqueId:               config.StateTopic,

			CommandTopic:           JoinStringsForTopic(m.selectPrefix, config.StateTopic, CmdTopicSuffix),
			// Options:                []string{"HOME", "CONFIG", "GATE", "DYN"},
			Options:                config.Options,
			Retain:                 true,
			ValueTemplate:          config.ValueTemplate,
			// CommandTemplate:        config.ValueTemplate,
			CommandTemplate:        fmt.Sprintf(`{"%s": "{{ value }}" }`, config.Name),
		}

		ct := JoinStringsForTopic(m.selectPrefix, uid, ConfigTopicSuffix)
...
func (m *Mqtt) PublishSelectValue(config EntityConfig) error {
	...
	for range Only.Once {
		...
		st = JoinStringsForTopic(m.selectPrefix, st, StateTopicSuffix)
...
func (m *Mqtt) SelectPublishValues(configs []EntityConfig) error {
	for range Only.Once {
		if len(configs) == 0 {
			break
		}
		...
		for i, config := range configs {
			...
			if topic == "" {
				topic = JoinStringsForTopic(m.selectPrefix, config.StateTopic, StateTopicSuffix)
			}
