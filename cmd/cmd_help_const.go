...
DefaultBinaryName - Over The Wire Behringer X32 to Gitlab syncing tool.

This tool does several things:
1. Pull a Gitlab repo that holds Behringer X32 data.
2. Fetch all data available from the Behringer X32.
3. Save this data as a JSON file.
4. Commit changes to the Gitlab repo.
5. Push changes to remote.

It is intended to provide full revision history for any changes made to the Behringer X32.

Use case example:
# Record changes made to user data on Behringer X32. (Will clone if not existing.)
	% DefaultBinaryName sync 'Updated users' users

# Record changes made to all Behringer X32 manually.
	% DefaultBinaryName sync 'Updated all zones'

# Record changes made to the Behringer X32 with default commit message.
	% DefaultBinaryName sync default

# Record changes made to the Behringer X32 via every 30 minutes.
...
# Config file.
	...
	Change Behringer X32 API token.
